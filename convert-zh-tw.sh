...
# =============================================================================
# Taiwan-specific Computer Terms
# =============================================================================
...
sed -i '' 's/台式電腦/桌機/g' "$TARGET_FILE"
sed -i '' 's/臺式電腦/桌機/g' "$TARGET_FILE"
...
sed -i '' 's/用戶/使用者/g' "$TARGET_FILE"

# Partition/Driver
sed -i '' 's/驱动程序/驅動程式/g' "$TARGET_FILE"
...
# Print/Open/OS (important Taiwan usage differences)
sed -i '' 's/打印機/印表機/g' "$TARGET_FILE"
sed -i '' 's/打印/列印/g' "$TARGET_FILE"
sed -i '' 's/打開/開啟/g' "$TARGET_FILE"
sed -i '' 's/操作系統/作業系統/g' "$TARGET_FILE"
sed -i '' 's/操作系统/作業系統/g' "$TARGET_FILE"

# Mirror/Image terminology
sed -i '' 's/鏡像源/鏡像站/g' "$TARGET_FILE"
sed -i '' 's/镜像列表/鏡像列表/g' "$TARGET_FILE"
...
# Comment out USTC, uncomment Taiwan mirrors in echo commands
...
# Fix inline comments in make.conf
...
# Remove mainland China specific hints
sed -i '' 's|（需翻牆）||g' "$TARGET_FILE"
sed -i '' 's|（可能需要國際網路）||g' "$TARGET_FILE"
sed -i '' 's|可能需要國際網路||g' "$TARGET_FILE"

# Change default sync-uri to GitHub
...
# Remove mainland China recommendation marks
