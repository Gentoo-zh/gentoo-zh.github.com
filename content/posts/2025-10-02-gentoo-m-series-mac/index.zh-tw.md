...
description: "2025 年最新 Apple Silicon Mac (M1/M2) Gentoo Linux 安裝指南，基於 Asahi Linux 專案，包含 Live USB 製作、分割區、核心安裝及桌面環境配置。M3/M4/M5 晶片暫不支援。"
---
...
**必選步驟**：
1. 下載官方 Gentoo Asahi Live USB 映象
...
**映象版本相容性資訊（更新日期：2025年11月21日）**

- **社群構建版本**：由 [Zakkaus](https://github.com/zakkaus) 基於 [gentoo-asahi-releng](https://github.com/chadmed/gentoo-asahi-releng) 構建的映象
...
2. **選擇分割區空間**：決定分配給 Linux 的空間（建議至少 80 GB）
...
Apple Silicon 的 Wi-Fi 已包含在核心中，應可正常運作。若不穩定，嘗試連線 2.4 GHz 網路。
...
ip a | grep inet          # 獲取 IP 位址
```

---

## 3. 分割區與檔案系統設定 {#step-3-partition}
...
**不要修改現有的 APFS 容器、EFI 分割區或 Recovery 分割區！** 只能在 Asahi 安裝程式預留的空間中操作。
...
```bash
lsblk
blkid --label "EFI - GENTO"  # 檢視你的 EFI 分割區
```
...
使用 `cfdisk` 進行分割區，它理解 Apple 分割區類別型並會保護系統分割區。
...
**傳統中文使用者推薦**：可以將上面的 `sync-uri` 改為北外源 `https://mirrors.bfsu.edu.cn/git/gentoo-portage.git` 以獲得更快的同步速度

更多映象源選項參考：[映象列表](/mirrorlist/)
...
如果 `etc-update` 出現配置檔案衝突，選擇 `-3` 進行自動合併。
...
- `rd.luks.uuid=<UUID>`：LUKS 加密分割區的 UUID（使用 `blkid /dev/nvme0n1p6` 獲取）
...
macOS 系統更新時會包含韌體更新。**建議保留 macOS 分割區**以便獲取最新韌體。
