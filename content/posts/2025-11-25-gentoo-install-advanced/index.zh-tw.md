...
date: 2025-11-25
summary: "Gentoo Linux 進階優化教學，涵蓋 make.conf 優化、LTO、系統維護等。"
description: "2025 年最新 Gentoo Linux 安裝指南 (進階優化篇)，涵蓋 make.conf 優化、LTO、系統維護等。"
keywords:
  - Gentoo Linux
  - make.conf
  - LTO
...
> 
> **系列導航**：
> 1. [基礎安裝](/zh-tw/posts/2025-11-25-gentoo-install-base/)：從零開始安裝 Gentoo 基礎系統
> 2. [桌面配置](/zh-tw/posts/2025-11-25-gentoo-install-desktop/)：顯示卡驅動、桌面環境、輸入法等
> 3. **進階優化（本文）**：make.conf 優化、LTO、系統維護
>
> **上一步**：[桌面配置](/zh-tw/posts/2025-11-25-gentoo-install-desktop/)
## 13. make.conf 進階設定指南

> **可參考**：[make.conf](https://wiki.gentoo.org/wiki//etc/portage/make.conf)

`/etc/portage/make.conf` 是 Gentoo 的全域設定檔，控制編譯器、最佳化參數、USE 旗標等。
...
| **桌面** | `wayland`, `X`, `gtk`, `qt6` | 桌面協議和工具包 |
| **音訊** | `pipewire`, `pulseaudio`, `alsa` | 音訊系統 |
| **視訊** | `ffmpeg`, `x264`, `vpx` | 視訊編解碼 |
...
# CPU 特性 (自動偵測，執行: emerge --ask app-portage/cpuid2cpuflags)
CPU_FLAGS_X86="<cpuid2cpuflags 輸出>"
```

> 詳見：[5.3 CPU 指令集優化 (CPU_FLAGS_X86)](/zh-tw/posts/2025-11-25-gentoo-install-base/#step-5-portage)
...
> **可參考**：[Upgrading Gentoo](https://wiki.gentoo.org/wiki/Upgrading_Gentoo/zh-cn) 和 [Gentoo Cheat Sheet](https://wiki.gentoo.org/wiki/Gentoo_Cheat_Sheet)
...
eclean-dist                         # 清理已下載的舊原始碼包
```

**6. 自動處理 USE 變更**
> **可參考**：[Autounmask-write](https://wiki.gentoo.org/wiki/Knowledge_Base:Autounmask-write) 和 [Dispatch-conf](https://wiki.gentoo.org/wiki/Dispatch-conf)

當安裝或更新軟體提示 "The following USE changes are necessary" 時：
1.  **讓 Portage 自動寫入設定**：`emerge --ask --autounmask-write <套件名>`
...
- **解決方法**：根據提示，手動卸載衝突軟體 (`emerge --deselect <套件名>` 後 `emerge --depclean`)。

**8. 安全檢查 (GLSA)**
Gentoo 發布安全公告 (GLSA) 來通知使用者潛在的安全漏洞。
```bash
glsa-check -l      # 列出所有未修復的安全公告
glsa-check -t all  # 測試所有受影響的軟體包
```

**9. 系統日誌與服務狀態**
定期檢查系統日誌和服務狀態，確保系統健康運作。
- **OpenRC**:
    ```bash
    rc-status      # 查看服務狀態
...
- **Systemd (Journalctl 常用指令)**:
    ...
    | `journalctl --disk-usage` | 查看日誌佔用的磁碟空間 |
    | `journalctl --vacuum-time=2weeks` | 清理 2 週前的日誌 |

### 13.2 Portage 技巧與目錄結構

> **可參考**：[Portage](https://wiki.gentoo.org/wiki/Portage/zh-cn) 和 [/etc/portage](https://wiki.gentoo.org/wiki//etc/portage/zh-cn)

**1. 核心目錄結構 (`/etc/portage/`)**
Gentoo 的設定非常靈活，建議使用**目錄**而不是單個檔案來管理設定：

| 檔案/目錄 | 用途 |
| --------- | ---- |
| `make.conf` | 全域編譯參數 (CFLAGS, MAKEOPTS, USE, GENTOO_MIRRORS) |
| `package.use/` | 針對特定軟體的 USE 旗標設定 |
| `package.accept_keywords/` | 允許安裝測試版 (keyword) 軟體 |
| `package.mask/` | 遮蔽特定版本的軟體 |
| `package.unmask/` | 解除遮蔽特定版本的軟體 |
| `package.license/` | 接受特定軟體的授權條款 |
| `package.env/` | 針對特定軟體的環境變數 (如使用不同的編譯器參數) |
...
| `--verbose` (`-v`) | 顯示詳細資訊 (USE 旗標等) | `emerge -av vim` |
| `--oneshot` (`-1`) | 安裝但不加入 World 檔案 (不作為系統依賴) | `emerge -1 rust` |
| `--update` (`-u`) | 更新軟體包 | `emerge -u vim` |
| `--deep` (`-D`) | 深度計算依賴 (更新依賴的依賴) | `emerge -uD @world` |
| `--newuse` (`-N`) | USE 旗標改變時重新編譯 | `emerge -uDN @world` |
| `--depclean` (`-c`) | 清理不再需要的孤立依賴 | `emerge -c` |
| `--deselect` | 從 World 檔案中移除 (不卸載) | `emerge --deselect vim` |
| `--search` (`-s`) | 搜尋軟體包 (推薦用 eix) | `emerge -s vim` |
...
2.  **搜尋軟體**：
    ...
    eix <關鍵詞>        # 搜尋所有軟體
    eix -I <關鍵詞>     # 僅搜尋已安裝軟體
    eix -R <關鍵詞>     # 搜尋遠端 Overlay (需設定 eix-remote)
    ```
...
> **注意**：`size` 大小不要超過你的實體記憶體大小（建議設為記憶體的一半），否則可能導致系統不穩定。
...
3. 針對特定軟體應用配置：
   編輯 `/etc/portage/package.env` (如果是目錄則創建檔案)：
   ```bash
...
詳細設定請參考 **Section 15 進階編譯最佳化**。

---

## 15. LTO 與 Clang 編譯最佳化 (可選)

> **風險提示**：LTO 會顯著增加編譯時間和記憶體消耗，且可能導致部分軟體編譯失敗。**強烈不建議全域開啟**，僅推薦針對特定軟體（如瀏覽器）開啟。
...
<details>
<summary><b>LTO 優缺點詳細分析（點擊展開）</b></summary>
...
> **新手提示**：如果你的系統是 4 核 CPU 配 4GB 記憶體，那麼花在編譯上的時間可能遠超最佳化帶來的效能提升。請根據硬體配置權衡利弊。

**1. 使用 USE 旗標開啟 (最推薦)**

對於 Firefox 和 Chromium 等大型軟體，官方 ebuild 通常提供了經過測試的 `lto` 和 `pgo` USE 旗標：
...
**USE="lto" 旗標說明**：部分軟體套件需要特殊修復才能支援 LTO，可以全域或針對特定軟體套件啟用 `lto` USE 旗標：
```bash
# 在 /etc/portage/make.conf 中全域啟用
USE="lto"
```
...
> **警告**：全域 LTO 會導致大量軟體套件編譯失敗，需要頻繁維護排除列表，**不建議新手嘗試**。
...
CFLAGS="${COMMON_FLAGS}"          # C 編譯器旗標
CXXFLAGS="${COMMON_FLAGS}"        # C++ 編譯器旗標
FCFLAGS="${COMMON_FLAGS}"         # Fortran 編譯器旗標
FFLAGS="${COMMON_FLAGS}"          # Fortran 77 編譯器旗標
LDFLAGS="${COMMON_FLAGS} ${LDFLAGS}"  # 連結器旗標

USE="lto"  # 啟用 LTO 支援的 USE 旗標
```

**4. 全域啟用 LTO (LLVM/Clang 系統 - 推薦使用 ThinLTO)**

> **預設推薦**：如果使用 Clang，強烈推薦使用 ThinLTO (`-flto=thin`) 而非 Full LTO (`-flto`)。ThinLTO 速度更快，記憶體占用更少，支援並行化。

> **警告**：如果 `clang-common` 未啟用 `default-lld` USE 旗標，必須在 `LDFLAGS` 中添加 `-fuse-ld=lld`。

編輯 `/etc/portage/make.conf`：
```bash
# Clang 目前尚未完全實現這些診斷，但保留這些旗標以備將來使用
...
| 類型 | 旗標 | 優勢 | 劣勢 | 推薦場景 |
|------|------|------|------|----------|
| **ThinLTO** | `-flto=thin` | • 速度快<br>• 記憶體占用少<br>• 支援並行化<br>• 編譯速度提升 2-3 倍 | • 僅 Clang/LLVM 支援 | **預設推薦**（Clang 使用者） |
| Full LTO | `-flto` | • 更深度的最佳化<br>• GCC 和 Clang 均支援 | • 速度慢<br>• 記憶體占用高<br>• 串行處理 | GCC 使用者或需要極致最佳化時 |

> **新手提示**：如果你使用 Clang，請務必使用 `-flto=thin`。這是目前的最佳實踐，能在保證效能的同時大幅減少編譯時間。
...
<details>
<summary><b>配置 1：停用 LTO 的軟體套件列表 (no-lto) - 點擊展開</b></summary>
...
建立 `/etc/portage/package.env/no-lto` 檔案（包含已知問題包）：
...
<details>
<summary><b>配置 2：低記憶體編譯模式 (low-memory) - 點擊展開</b></summary>
...
> **提示**：如果遇到其他 LTO 相關的連結錯誤，請先嘗試停用該包的 LTO。也可以查看 Gentoo Bugzilla 是否有相關報告。
...
> - Gentoo 維護了 [bug #408963](https://bugs.gentoo.org/408963) 來追蹤 Clang 編譯失敗的軟體套件。

**1. 針對特定軟體開啟 (推薦)**

建立環境設定檔 `/etc/portage/env/clang.conf`：
```bash
CC="clang"
CXX="clang++"
...
> **注意**：如果需要 PGO 支援（如 `dev-lang/python[pgo]`），需要安裝以下套件：
...
在 `/etc/portage/package.use` 中啟用相關 USE 旗標：
```text
...
> **警告**：
> - 如果沒有啟用 `profile` 和 `orc` USE 旗標，帶有 `pgo` USE 旗標的軟體套件（如 `dev-lang/python[pgo]`）會編譯失敗。
> - 編譯日誌可能會報錯：`ld.lld: error: cannot open /usr/lib/llvm/18/bin/../../../../lib/clang/18/lib/linux/libclang_rt.profile-x86_64.a`

**4. 全域開啟 (不建議初學者)**

全域切換到 Clang 需要系統大部分軟體支援，且需要處理大量相容性問題，**僅建議進階使用者嘗試**。

如需全域啟用，在 `/etc/portage/make.conf` 中添加：
```bash
CC="clang"
CXX="clang++"
CPP="clang-cpp"
...
> **新手提示**：`localmodconfig` 是最安全的方法，它會確保你的硬體都能正常工作，同時移除不需要的驅動。
...
| 英文選項 | 中文說明 | 關鍵配置 |
| :--- | :--- | :--- |
| **General setup** | 一般設定 | 本地主機名稱、Systemd/OpenRC 支援 |
| **Processor type and features** | 處理器類型與特性 | CPU 型號選擇、微碼載入 |
| **Power management and ACPI options** | 電源管理與 ACPI | 筆記型電腦電源管理、暫停/休眠 |
...
| **Networking support** | 網路支援 | TCP/IP 協定堆疊、防火牆 (Netfilter) |
| **Device Drivers** | 裝置驅動 | 顯示卡、網路卡、音效卡、USB、NVMe 驅動 |
...
> **重要建議**：對於手動編譯，建議將**關鍵驅動**（如檔案系統、磁碟控制器、網卡）直接編譯進核心（選擇 `[*]` 或 `<*>` 即 `=y`），而不是作為模組（`<M>` 即 `=m`）。這樣可以避免 initramfs 缺失模組導致無法啟動的問題。
...
5. **Gentoo Linux 專有選項**（推薦全部啟用）：
   ...
   > **新手提示**：啟用 "Select options required by Portage features" 可以自動配置大部分必需選項，非常推薦！

> **提示**：在 menuconfig 中，按 `/` 可以搜尋選項，按 `?` 檢視幫助。
...
```
解決：某些模組不相容 LTO，禁用 LTO 或將問題模組設為 `=y`（而非 `=m`）
...
本節適用於需要配置軟體 RAID (mdadm) 的伺服器使用者。
...
> **提示**：配置完成後，務必檢查 `/boot/initramfs-*.img` 是否包含 RAID 模組：
