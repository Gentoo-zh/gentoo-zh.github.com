...
date: 2025-11-25
summary: "Gentoo Linux 桌面環境配置教學，涵蓋顯示卡驅動、KDE/GNOME/Hyprland、輸入法、字體等。"
description: "2025 年最新 Gentoo Linux 安裝指南 (桌面配置篇)，涵蓋顯示卡驅動、KDE/GNOME/Hyprland、輸入法、字體等。"
keywords:
  - Gentoo Linux
  - KDE Plasma
  - GNOME
...
>
> **上一步**：[基礎安裝](/zh-tw/posts/2025-11-25-gentoo-install-base/)
...
**設定 VAAPI 影片加速**
> **可參考**：[VAAPI](https://wiki.gentoo.org/wiki/VAAPI) 和 [nvidia-vaapi-driver](https://packages.gentoo.org/packages/media-libs/nvidia-vaapi-driver)
...
2. **安裝驅動與工具**：
   ...
   **NVIDIA 使用者特別步驟**：
   ...
   ```
   > **注意**：`nvidia-vaapi-driver` 在 Wayland 下可能存在不穩定性（如 CUDA/OpenGL 互操作問題）。
...
3. **驗證**：
   執行 `vainfo` 查看輸出，若無錯誤且顯示支援的 Profile 即為成功。
...
> **可參考**：[PipeWire](https://wiki.gentoo.org/wiki/PipeWire/zh-cn) 和 [Bluetooth](https://wiki.gentoo.org/wiki/Bluetooth/zh-cn)
...
> **更多選擇**：如需查看其他桌面環境，請參考 [Desktop environment](https://wiki.gentoo.org/wiki/Desktop_environment/zh-cn)。
...
2. **啟用**
   進入 GNOME 設定 -> 鍵盤 -> 添加輸入來源 -> 選擇 "Chinese (Rime)"。
...
**3. 添加 Gentoo-zh Overlay**
   在 `/etc/portage/repos.conf/` 目錄下創建 `gentoo-zh.conf` 檔案，內容如下：
   ```ini
   [gentoo-zh]
...
2. **啟用軟體中心支援**
   ...
   **GNOME 使用者**：
   在 `/etc/portage/package.use/gnome` (或新建檔案) 中添加：
   ```conf
   gnome-extra/gnome-software flatpak
   ```

   **KDE 使用者**：
   在 `/etc/portage/package.use/kde` (或新建檔案) 中添加：
   ```conf
...
> **可參考**：[Zram](https://wiki.gentoo.org/wiki/Zram)
> **推薦**：Zram 可以創建壓縮的記憶體交換分區，有效防止編譯大型軟體時記憶體不足 (OOM)。
...
> **下一步**：[進階優化](/zh-tw/posts/2025-11-25-gentoo-install-advanced/)
